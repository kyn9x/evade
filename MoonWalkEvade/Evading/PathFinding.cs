...
namespace MoonWalkEvade.Evading
{
    ...
    {
        public MoonWalkEvade _moonWalkEvade;

        public PathFinding(MoonWalkEvade moonWalkEvade)
        {
        ...
        public Vector2[] CreatePathOld(Vector2 start, Vector2 end)
        {
            ...
            var walkPolygons = Geometry.ClipPolygons(_moonWalkEvade.Skillshots.Select(c => c.ToPolygon(extraWidth))).ToPolygons();

            //if (walkPolygons.Any(pol => pol.IsInside(start)))
            //{
            //    var polPoints =
            //        Geometry.ClipPolygons(
            //            _moonWalkEvade.SpellDetector.DetectedSkillshots.Where(c => c.IsValid)
            ...
            while (pathPoints.Count > 0)
            {
                    ...
                    pol1.Equals(pol2))
                {
                    ...
                    var linkedList = new Utils.LinkedList<Vector2>(detailedPolygon.Points, index);
        ...
        public Vector2[] CreatePath(Vector2 start, Vector2 end)
        {
            ...
            var polygons = _moonWalkEvade.ClippedPolygons.OrderByDescending(p => p.CenterOfPolygon().Distance(Player.Instance, true)).Reverse().ToArray();
        ...
        public Vector2[] CleanPath(Vector2[] path)
        {
            ...
            {
                if (_moonWalkEvade.ClippedPolygons.Any(p => p.IsIntersectingWithLineSegment(current, path[i])))
                {
                    newPath.Add(path[i]);
                    current = path[i];
                }
